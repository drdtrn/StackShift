...
public interface IIncidentRepository : IRepository<Incident, Guid>
{
    ...
    Task<IReadOnlyList<Guid>> GetOrganizationIdsWithIncidentsSinceAsync(DateTimeOffset since, CancellationToken ct = default);
    Task<IReadOnlyList<Incident>> GetCreatedSinceByOrgIdAsync(Guid organizationId, DateTimeOffset since, CancellationToken ct = default); 
}
